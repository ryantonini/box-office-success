...
class BaggingFNN():
    ...
    This class uses the neural network implementation within the PyBrain library.
    The constructor requires a feed-forward neural network as an argument, as 
    defined within the PyBrain library.
    
    Specific metric and cross-validation tools from within the Scikit Learn 
    library are also used by this class.  
    """
    ...
    def cross_validate(self, data, folds=4):
        ...
        for train_index, test_index in skf:  # iterate each fold
            ...
            s = self.bagging_classifier(ds_train, ds_test, 
                                                  len(ds_train['input']))
            #s = fnn_classify(ds_train, ds_test)
            print s
            total+=s
        return total/folds
    
    def generate_dataset(self, X, y):
        """Return a classification dataset based on features X and targets y"""
